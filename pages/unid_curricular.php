...
$stmt_check = $conn->prepare("
    ...
    WHERE nombre = ? 
      ...
      AND id_sede = ?
");
$stmt_check->bind_param("ssii", $nombre, $grupo, $id_periodo, $_POST['id_sede']);
...
<div class="container mt-5">
    <div class="card shadow-sm border-0 rounded-4">
        <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center rounded-top-4">
            ...
            <button class="btn btn-light btn-sm fw-semibold" data-bs-toggle="collapse"
                data-bs-target="#formRegistroUnidad">
                ➕ Nueva Unidad
            </button>
        </div>
                ...
                <form id="formUnidad" class="row g-3">
                    ...
                    <div class="col-md-3">
                        ...
                        <select id="select_catalogo_unidad" class="form-select" required>
                            ...
                            <option value="<?= $cat['id'] ?>" data-valor="<?= $cat['valor_base'] ?>">
                    ...
                    <div class="col-md-2">
                        ...
                        <input type="number" step="0.01" name="valor" id="input_valor_unidad" class="form-control"
                            required readonly>
                    </div>
                    <div class="col-md-3">
                        ...
                        <div class="d-flex">
                            ...
                            <button type="button" class="btn btn-outline-secondary" data-bs-toggle="modal"
                                data-bs-target="#modalPeriodo">+</button>
                        </div>
                    </div>
                    <div class="col-md-2">
    ...
    <div class="card mt-4 shadow-sm border-0 rounded-4">
        <div class="card-body">
            <div class="mb-3">
                <input type="text" id="filtroTabla" class="form-control shadow-sm"
                    placeholder="🔍 Buscar por nombre, grupo, cohorte o sede...">
            </div>
                            ...
                            <td class="text-center">
                                <div class="btn-group btn-group-sm">
                                    <button class="btn btn-outline-warning"
                                        onclick='editarUnidad(<?= json_encode($u) ?>)'>Editar</button>
                                    <button class="btn btn-outline-danger"
                                        onclick="eliminarUnidad(<?= $u['id_unidad'] ?>)">Eliminar</button>
                                </div>
                            </td>
                        </tr>
                        <?php endwhile; ?>
...
document.getElementById('formEditar').addEventListener('submit', e => {
    ...
    fetch(location.href, {
        ...
        .then(res => res.json())
        .then(data => {
            if (data.success) Swal.fire('Actualizado', 'Unidad actualizada con éxito', 'success').then(() =>
                location.reload());
...
document.getElementById('formUnidad').addEventListener('submit', e => {
    ...
    fetch(location.href, {
        ...
        .then(data => {
            if (data.success) {
                Swal.fire('Éxito', data.msg || 'Unidad registrada correctamente', 'success').then(() =>
                    location.reload());
            } else {
